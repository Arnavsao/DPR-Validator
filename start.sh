...
if [ ! -f "$DPR_PDF" ]; then
    echo "   [WARNING] DPR format Vol-I.pdf not found at: $DPR_PDF"
...
" 2>/dev/null || echo "error")
    ...
    if [ "$KB_READY" = "yes" ]; then
        echo "   [OK] Knowledge base already populated."
    elif [ "$KB_READY" = "error" ]; then
        echo "   [WARNING] Could not check KB status (chromadb may not be installed yet)."
...
if [ -f "$BACKEND_DIR/storage/dpr_validator.db" ]; then
    ...
    cd "$BACKEND_DIR"
    python migrate_add_rag_fields.py 2>/dev/null && echo "   [OK] DB up to date." || echo "   [WARNING] Migration skipped (fresh DB)."
    cd "$SCRIPT_DIR"
fi
